...
for tune in  CV2ElasticNuwroSFn CV2ElasticNuwroSF CV2ElasticNuwroSFnz CV2ElasticNuwroSFz 
do
  #playlist=All
  ...
  output=${folder}/signal_weights_ptmu_minervameNu_q2qe_flux_2D_optimized_Type_weighted_w_lambda_${signalweight}_${lambda}_qe_res_2p2h_snp_scp.root

  savefolder=$folder
  #mkdir -p $savefolder
  #ln -sf $folder/*root ${savefolder}
  #cmd="./SideBandFitAngleAllSamples_Neutron $signal $blob $michel $micblob  $output ${signalweight} ${lambda}"
  cmd="./SideBandFitAngleAllSamples_Neutron_ptmu $signal $blob $michel $micblob  $output ${signalweight} ${lambda}"
  echo $cmd
  #$cmd
