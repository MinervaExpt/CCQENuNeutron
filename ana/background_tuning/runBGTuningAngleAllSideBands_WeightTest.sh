...
for tune in CV2ElasticNuwroSFn #CV CV2 CV2Elastic pion_rpa default
do
  #playlist=All
  folder=../rootfiles/minervameAntiNu/cuts${cuts}/${tune}/
  signal=$folder/MuonEventSelection_MakeFlux-1_Multiplicity-1_Sample-Signal-Angles_CombinedPlaylists.root
  blob=$folder/MuonEventSelection_MakeFlux-1_Multiplicity-1_Sample-BlobSideBand-Angles_CombinedPlaylists.root
  michel=$folder/MuonEventSelection_MakeFlux-1_Multiplicity-1_Sample-MichelSideBand-Angles_CombinedPlaylists.root
  micblob=$folder/MuonEventSelection_MakeFlux-1_Multiplicity-1_Sample-MicBlobSideBand-Angles_CombinedPlaylists.root
  ...
  do
    ...
    do
      ...
      cmd="./SideBandFitAngleAllSamples_test $signal $blob $michel $micblob  $output $w $lam"
      echo $cmd
      $cmd 
    done
  done
