...
cuts="_muonPCut-0_maxClusECut-0_vtxCut-0_blobMaxECut-0_n2DCut-0_nBlobCut-1_newBlobCut-1_ElasticQE"
cuts="_baseCut_chooseTotalE"
...
files = glob.glob("%s/test/test_*_qe_res_2p2h_scp_snp_noNorm_blob99.root"%inputfolder);
print(files)
...
minw,minlam,minchi2, mindchi2 = -1,-1,999999999999,999999999
...
for i,fpath in enumerate(files):
  ...
  for cat in ["qelike_qe_oth", "qelike_res", "qelike_2p2h", "qelikenot_scp", "qelikenot_snp"]:
    h=f.Get("hs_weights_yvar_bgType_%s"%cat)
    fits.append(h)
    #print fits[-1].GetName()
  ...
  print(w,lam,chi2)
  dchi2 = abs(chi2-1)
  if dchi2< mindchi2 and lam<1000:
    mindchi2 = dchi2
    minchi2 = chi2
    minw = w
    minlam=lam
