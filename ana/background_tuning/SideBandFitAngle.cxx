...
void getAllUniverseScalesSingleType1D( vector<MnvH1D**> &tracks, vector<MnvH1D*> &track_scale)
{
...
int SideBandFit( vector<string> filenames )
{
  ...
  vector<MnvH1D**>  h_fit_varbins_samples;
  for( int i = 0; i<n_samples; i++ ) 
  {
    MnvH1D *h[nHistos];
    h_fit_varbins_samples.push_back( h );
  }
  ...
  for( int i = 0; i<n_samples;i++)
  {
    ...
    //cout<<"Subtracting Non-QELike background on Sample "<<i<<endl;
    //h_fit_varbins_samples[i][kData]->Add( h_fit_varbins_samples[i][kQELikeNot], -1 );
  }
