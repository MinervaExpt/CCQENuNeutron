...
vector<int> categories_to_fit({ kQELike_QE_OTH, kQELike_RES, kQELike_2p2h,kQELikeNot_SingleNeutralPion, kQELikeNot_SingleChargedPion});
vector<string> categories_to_fit_names({"qelike_qe_oth", "qelike_res","qelike_2p2h", "qelikenot_snp", "qelikenot_scp"});
...
const int n_fit_bins = 9;
int fit_bins_start[n_fit_bins] = {1,2,3,4,5,6,7,8,14};
int fit_bins_end[n_fit_bins] = {2,3,4,5,6,7,13,14};
...
void minuitFunction( int &nDim, double* gout, double &result, double par[], int flg)
{
  result = chi2Function(par)+lambda*regularizer(par)+20*constrainer(par) ;
  //cout<<result<<endl;
}
...
vector<TH1D*> GetSingleTypeFitScalesAndErrors()
{
  ...
  double v = 0.8, verr=0.0000001, vmin = 0, vmax = 10;
  for(int i=0;i<n_fit_bins;i++)
  {
...
int SideBandFit( vector<string> filenames )
{
  ...
  ExpandHistos( hs_vect_yvar_scales, hs_vect_blobN_scales, 1);
  ExpandHistos( hs_vect_yvar_scales, hs_vect_blobType_scales, 1);

  ExpandHistos( hs_vect_yvar_scales, hs_vect_muontheta_scales, 1);
  ExpandHistos( hs_vect_yvar_scales, hs_vect_muonmomentum_scales, 1);
