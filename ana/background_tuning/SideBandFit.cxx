...
const int n_fit_bins = 9;
int fit_bins_start[n_fit_bins] = {1,6,7,8,9,10,11,12,13};
int fit_bins_end[n_fit_bins] =   {5,6,7,8,9,10,11,12,18};
...
int SideBandFit( vector<string> filenames )
{
  ...
  TVector2 *pot = (TVector2*) fin[0]->Get("pot");
  //int nPlaylists = 12;
  //for( int i = 2; i<nPlaylists+1; i++ ) (*pot)+= *((TVector2*)fin[0]->Get(Form( "pot;%d", i) ));
  ...
  for( int i = 0; i<n_samples; i++ ) 
  {
    ...
    hname = Form( "h_weights_dthetaPdthetaR_yvarbins_qelikenot_%s", Samples[i].c_str() ) ;
  ...
  ExpandHistos( h_vect_yvar_scales, h_vect_dthetaPdthetaR_scales, 1);
  ...
  for( int i = 0; i<nTypes; i++ ) 
  {
    ...
    hs_vect_dthetaPdthetaR_scales.push_back( new MnvH2D( hname.c_str(), hname.c_str(), global_x.nbins, &(global_x.bin_edges[0]), ybins.nbins, &( ybins.bin_edges[0] ) ) );
  ...
  for(int i=0;i<n_samples;i++){
    ...
    h_vect_dthetaPdthetaR_scales[i]->Write();
