...
double AnalysisLooperBase(T* evt, CCQENuUtils* &utils, CCQENuCuts* cutter, NeutronBlobCuts* ncutter, string sample, int multiplicity, int n_entries, map<int,int> &cuts_counter, bool isData=false, bool isTruth=false, int npMode = 1)
{
  ...
  for( int i = 0; i< n_entries; ++i)
  {
   ...
   XYZTVector reco_muon_XYZT(evt->CCQENu_leptonE[0]/1000.,
    ...
    ////Set main neutron candidate
    ptrNeutronCandidates = ncutter->GetCandidates();
    mainCandidate = ncutter->MainNeutronCandidate();

    int candidatePID = mainCandidate->MCPID;
    int candidateParentPID = mainCandidate->ParentMCPID;
    cout<<"ParentMCPID == "<<candidateParentPID<<endl;
    double totalE = mainCandidate->TotalE;
    ...
    {
      if( candidatePID == 2212 ) utils->fillHistosV3( utils->histos1D["h_blobTotalE_pid2212"], totalE, isData, evt, wgt );
      if( candidateParentPID== 2112 ) utils->fillHistosV3( utils->histos1D["h_blobTotalE_parentpid2112"], totalE, isData, evt, wgt );
      if( candidatePID == 2212  && candidateParentPID== 2112 )
        utils->fillHistosV3( utils->histos1D["h_blobTotalE_parentpid2112_pid2212"], totalE, isData, evt, wgt );
      utils->fillHistosV3( utils->histos1D["h_blobTotalE_total"], totalE, isData, evt, wgt );
...
int MuonSelectionHists(string filename, string sample, bool makeFluxConstraintHisto, int multiplicity, string playlist, int n_mcfiles = -1, int n_datafiles = -1, int npMode = 1)
{
  ...
  MnvH1D *h_blobTotalE_pid2212[nHistos];
  MnvH1D *h_blobTotalE_parentpid2112[nHistos];
  MnvH1D *h_blobTotalE_parentpid2112_pid2212[nHistos];
  MnvH1D *h_blobTotalE_total[nHistos];
  utils->bookHistos( h_blobTotalE_pid2212, "h_blobTotalE_pid2212", "blobTotalE (MeV)", 500,0,1000 );
  utils->bookHistos( h_blobTotalE_parentpid2112, "h_blobTotalE_parentpid2112", "blobTotalE (MeV)", 500,0,1000 );
  utils->bookHistos( h_blobTotalE_parentpid2112_pid2212, "h_blobTotalE_parentpid2112_pid2212", "blobTotalE (MeV)", 500,0,1000 );
  utils->bookHistos( h_blobTotalE_total, "h_blobTotalE_total", "blobTotalE (MeV)", 500,0,1000 );
