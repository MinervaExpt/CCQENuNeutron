...
#include <map>
  ...
  {
    string tag = tags[it];
    TFile* f = new TFile(Form("../../root_files/inelastic_graph_abf_inela_%s_CombinedPlaylists.root",tag.c_str()),"read");
    ...
    map<double, TH1D*> h_blobE1D_data, h_blobDist1D_data, h_nBlobs1D_data;
    double nmc = 0;
    for( int i = 0; i< n; i++ )
    {
      cout<<i<<endl;
      ...
      h_nBlobs[w] = (TH2D*) f->Get( Form("h_nBlobs_q2qe_%0.1f_%s", type.c_str(), w ) );
      ...
      h_nBlobs1D_data[w] = (TH1D*) h_nBlobs_data[w]->ProjectionX();
    ...
    opt = "hist";
    int imin = 0;
    TH1D* nBlobs = (TH1D*) h_nBlobs1D[1]->Clone("nBlobs");
    TLegend *l2 = new TLegend( .7, .6, 1,1 );
    for( int i = imin; i< n; i++ )
    {
      double w = rws[i];
      double chi2=0;
      for( int j = 1; j <= h_nBlobs1D[w]->GetNbinsX(); j++ )
      {
        double vmc = h_nBlobs1D[w]->GetBinContent(j);
        double vdata = h_nBlobs1D_data[1]->GetBinContent(j);
        if( h_nBlobs1D_data[1]->FindBin( 1500 ) == j ) break;
        chi2+= ( vmc - vdata )*( vmc - vdata )/(  vdata ) ;
        //cout<<vmc<<", "<<vdata<<endl;
      }

      h_nBlobs1D[w]->Divide( nBlobs );
      h_nBlobs1D[w]->GetYaxis()->SetRangeUser(0.5,1.5);
      h_nBlobs1D[w]->GetXaxis()->SetRangeUser(0,2500);
      h_nBlobs1D[w]->GetXaxis()->SetTitle("Blob Dist (mm)");
      h_nBlobs1D[w]->Draw(opt.c_str() );
      if ( i==imin ) opt+="same";
      l2->AddEntry(h_blobE1D[w], Form("%.1f, #chi^{2}=%.2f",w,chi2 ) );
    }
    if(type=="mc")
    {
      h_nBlobs1D_data[1]->Divide( nBlobs );
      h_nBlobs1D_data[1]->Draw("esame");
    }
    l2->Draw();
    //h_nBlobs[0]->Draw("colz");
    c->SetGrid();
    c->Print(Form("BlobN_%s_%s.pdf",type.c_str(), tag.c_str() ));
