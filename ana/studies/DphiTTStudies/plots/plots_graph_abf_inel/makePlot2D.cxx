  ...
  for( int it = 0; it<tags.size(); it++)
  {
   ...
   // double rws[] = {0, .5,.6,.7,.8,.9,1,1.1,1.2,1.3,1.4,1.5,2,3};
    ...
    for( int i = 0 ; i< n; i++ )
    {
      ...
      h_nBlobs_data[w]->GetXaxis()->SetTitle("Dist (mm)");
    ...
    {
      double w = rws[i];
      h_nBlobs[w]->Divide( h_nBlobs_base );
      for( int j = 0; j< NPlots; j++ )
      {
    ...
    h_nBlobs_data[1]->Divide( h_nBlobs_base);
    for( int j = 0; j< NPlots; j++ )
    {
      c->cd(j+1);
      if(type == "mc")
    ...
    //c->SetGrid();
    c->Print(Form( "BlobN2D_%s_%s.pdf", type.c_str(), tag.c_str()) );
