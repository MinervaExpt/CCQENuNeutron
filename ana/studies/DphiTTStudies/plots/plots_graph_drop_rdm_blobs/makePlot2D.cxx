...
#include <map>
  ...
  {
    string tag = tags[it];
    TFile* f = new TFile(Form("../../root_files/inelastic_graph_drop_0d3_%s_CombinedPlaylists.root",tag.c_str()),"read");
    //TFile* f = new TFile(Form("../../root_files/inelastic_graph_abf_%s_minervame6A.root",tag.c_str()),"read");
   ...
   // double rws[] = {0, .5,.6,.7,.8,.9,1,1.1,1.2,1.3,1.4,1.5,2,3};
    double rws[] = {0,0.1,0.3,.5,.6,.7,.8,.9,1,1.1,1.2,1.3,1.4,1.5,1.7,2};
    int n = sizeof(rws)/sizeof(rws[0]) ;
    //vector<double> reweights( rws, rws+ n );
    map<double, TH2D*> h_blobE, h_blobDist, h_nBlobs;
    map<double, TH2D*> h_blobE_comp, h_blobDist_comp, h_nBlobs_comp;
    map<double, TH2D*> h_blobE_data, h_blobDist_data, h_nBlobs_data;
    map<double, TH1D*> h_blobE1D, h_blobDist1D, h_nBlobs1D;
    ...
    for( int i = 0 ; i< n; i++ )
    {
      ...
      h_nBlobs_comp[w] = (TH2D*) f->Get( Form("h_nBlobs_q2qe_%0.1f_%s",  w, comp.c_str() ) );
      ...
      h_nBlobs[w] = (TH2D*) f->Get( Form("h_nBlobs_q2qe_%0.1f_%s", w, type.c_str() ));
    ...
    if(doRatio) h_blobE_data[1.0]->Divide( h_blobE_base );
    if(doRatio) h_blobE_comp[1.0]->Divide( h_blobE_base );
    for( int j = 0; j< NPlots; j++ )
    {
      c->cd(j+1);
      if(type == "mc")
    ...
    if(doRatio) h_blobDist_data[1]->Divide( h_blobDist_base);
    for( int j = 0; j< NPlots; j++ )
    {
      c->cd(j+1);
      if(type == "mc")
    ...
    opt="histl";
    TH2D* h_nBlobs_base = (TH2D*) h_nBlobs[1.0]->Clone( "h_nBlobs_base" );
    for( int i = imin; i< n; i++ )
    {
      double w = rws[i];
      ...
      for( int j = 0; j< NPlots; j++ )
      {
        TH1D* h = (TH1D*) h_nBlobs[w]->ProjectionX(Form("%.1f_%d",w,j+1), j+1,j+1);
        h->GetXaxis()->SetRangeUser(0,2500);
        h->SetLineColor(i+1);
    ...
    if(doRatio) h_nBlobs_data[1]->Divide( h_nBlobs_base);
    for( int j = 0; j< NPlots; j++ )
    {
      c->cd(j+1);
      if(type == "mc")
      {
        TH1D* hdata = (TH1D*) h_nBlobs_data[1]->ProjectionX(Form("data_%.1f_%d",w,j+1), j+1,j+1);
        hdata->SetMarkerStyle(0);
        hdata->Draw("esame");
        hdata->Draw("esame");
      }
