...
// =====================================================================
void makePlots(bool pt, string location, string variable)
{
  ...
  TFile f(Form("%s_CV/CrossSection_CombinedPlaylists.root_%s.root",location.c_str(),variable.c_str()));
  MnvH2D* data=(MnvH2D*)f.Get(Form("h_%s_ptmu_data_nobck_unfold_effcor_cross_section",variable.c_str()));
  ...
  TLegend *leg = new TLegend(0.8, 0.1, 0.95, 0.3);
  leg->SetLineColor(kWhite);
  leg->SetFillColor(kWhite);
  for(int i=0;i<histsPT2D.size();i++)leg->AddEntry(histsPT2D[i].first,Sysnames[i].c_str(),"l");
  if(pt){
    GridCanvas* gcPT=plotYAxis1D(histsPT2D,"p_{t} (GeV) ",variable.c_str());
    ...
    GridCanvas* gcPT=plotXAxis1D(histsPT2D,variable.c_str(),"p_{t} (GeV)");
    gcPT->SetYTitle("Fractional uncertainty");
    gcPT->SetYLimits(0, 0.49);
    leg->Draw("SAME");
    gcPT->Modified();
