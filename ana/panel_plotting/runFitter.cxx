...
double ProcessResults()
{
  ...
  double minY=999999;
  vector<double> x;
  for( int i=-10; i<-1;i++ ) x.push_back( TMath::Power(10,i*1.0) );
  for( int i = 1; i< 10; i++ ) x.push_back(0.1*i );
  x.push_back(1);
  for( int i = 1; i< 10; i++ )x.push_back(1+0.1*i);
  for( int i = 2; i< 10; i++ )x.push_back( i);
  for( int i=1; i<10;i++ ) x.push_back( TMath::Power(10,i*1.0) );
  vector<double> y;
