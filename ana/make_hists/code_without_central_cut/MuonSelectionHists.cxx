...
double AnalysisLooperBase(T* evt, CCQENuUtils* &utils, CCQENuCuts* cutter, NeutronBlobCuts* ncutter, string sample, int multiplicity, int n_entries, map<int,int> &cuts_counter, bool isData=false, bool isTruth=false, int npMode = 1)
{
  ...
  for( int i = 0; i< n_entries; ++i)
  {
    ...
    if(npMode == 0)
    {
      if (EventCutWithoutNeutron( evt, cutter, ncutter, cuts_counter, multiplicity, sample ,  isData, npMode))
      {
        ...
        if (recoilE<1)
        {
          ...
          XYZTVector reco_muon_4P( evt->CCQENu_leptonE[0]/1000., evt->CCQENu_leptonE[1]/1000., evt->CCQENu_leptonE[2]/1000. ,evt->CCQENu_muon_E/1e3);
    ...
    //if( evt->recoil_energy_nonmuon_nonvtx100mm<5)
    ...
    bool mainCandidateClusMaxECut = mainCandidate->ClusMaxE > 10;
    bool mainCandidateTotalECut = (mainCandidate->TotalE > 10);
    //if( npMode !=1 && !mainCandidateTotalECut && useTotalECut ) continue;
    ...
    if( modify_Eb && isMC ) reco_muon_4P = kineMod->GetCorrectedMuon( evt, reco_muon_4P, mode_eb );
    XYZVector reco_muon_3P = (XYZVector) reco_muon_4P.Vect();
    
    
    //double muon_T         =  evt->CCQENu_muon_T/1000.; //GeV
    double muon_T         =  reco_muon_4P.E() - reco_muon_4P.M();
    ...
    double muon_phi_detector = ATan2( reco_muon_py, reco_muon_px )*180./3.14159;
    ...
    if (npMode != 0 )
    {
      ...
      if( evt->CCQENu_proton_Px_fromdEdx == evt->CCQENu_proton_Py_fromdEdx == evt->CCQENu_proton_Pz_fromdEdx == -9999 ) hasProton = false;
      if( !hasProton ) continue;
    ...
    XYZVector primaryNucleonVect_XYZ= Convert( primaryNucleonVect );
    ...
    double q0 = enu - reco_muon_4P.E();
    double q3 = pow(q0*q0+q2qe,.5);

    //cout<<q2qe<<", "<<enu<<", "<<reco_muon_4P.E()<<", "<<q0<<", "<<q3<<endl;
    //if( enu > 10 ) continue;
    ...
    if(isMC && modify_Eb && npMode != 0) primaryNucleonVect_XYZT = kineMod->GetCorrectedNucleon( evt, primaryNucleonVect_XYZT , mode_eb );
    ...
    if(npMode == 0)
    {
      ...
      XYZVector expected2D = ( ncutter->DetectorUtilsPtr()->ViewProjection2D( expectedNucleon3P, view2 ) ).Unit();


      double dtheta2D = geoUtils->ComputeNeutronAngle2D( nu2D, expected2D, blobDir2D)*180/3.14159;
    }

    //=================SetTransverseParticles=========================================
    utils->setTransverseParticle( primaryNucleonVect, 0, false );
    ...
    XYZVector fitNeutron = blobVtxDir;
    if( hasNC && hasProton  && npMode == 10) 
    {
      quadrant = GetQuadrantCode( beam, reco_muon_3P, protonVectXYZ, blobVtxDir );
      //cout<<quadrant<<endl;

      vector<double> fitMomentum = geoUtils->FitMomentum( beam, reco_muon_3P, protonVectXYZ, (XYZVector) blobVtxDir.Unit(), geoUtils->MnGeV() );
      pneutron = fitMomentum[0];
      if (pneutron>0)
      {
        pneutronErr = fitMomentum[1];
        XYZVector fitNeutron = blobVtxDir*abs(fitMomentum[0]);
        ...
        EnuFit = beam.Dot( reco_muon_3P+protonVectXYZ+fitNeutron );

        std::vector<double> ddtransverseVars = geoUtils->GetTransverseVariables( beam, reco_muon_4P+primaryNucleonVect_XYZT, fitNeutron_XYZT, Mcarbon, 27.13/1E3);
        dphitt = ddtransverseVars[vdphiT];
