...
double AnalysisLooperBase(T* evt, CCQENuUtils* &utils, CCQENuCuts* cutter, NeutronBlobCuts* ncutter, string sample, int multiplicity, int n_entries, map<int,int> &cuts_counter, bool isData=false, bool isTruth=false, int npMode = 1)
{
  ...
  for( int i = 0; i< n_entries; ++i)
  {
    ...
    if(isDebug) cout<<"Debug Event "<<i<<endl;
    //==============Fill Neutron Blobs======================
    bool isMC = !isData;
    if(testMode)
    {
    ...
    //Cuts concatenated in EventCut:
    //neutron candidate cut
    //neutron muon cut
    //michel and nblob
    //recoil cut
    // int vertex, deadtime, helicity, trackangle
    //
    ...
    //cuts passing muon reco
    // muon momentum 1.5,20
    // redundant neutron candidate + neutron muon cut
    ...
    if( npMode != 1 )
    {
      bool PassRecoilBlobCutLoose = (npMode==0)? PassRecoilBlobQsqCutsLoose(evt, cutter, ncutter ) : true;
      if(!PassRecoilBlobCutLoose) continue;
      pass_cv = (npMode == 0)? PassRecoilBlobQsqCutsCV(evt, cutter, ncutter ) : true;
    }else
    {
      if(!cutter->passSignalFunction( evt, 0, 0 )) continue;
    ...
    if(npMode == 1) hasNC = false;

    NeutronCandidates* ptrNeutronCandidates=NULL; 
    NeutronBlob* mainCandidate=NULL; 

    hasNC *= (npMode != 1 );
    if (hasNC) 
    {
      if (isDebug) cout<<"set mainCandidate"<<endl;
      ptrNeutronCandidates = ncutter->GetCandidates();
      mainCandidate = ncutter->MainNeutronCandidate();
      if (isDebug)
      {
        cout<<ptrNeutronCandidates<<endl;
        cout<<mainCandidate<<endl;
    ...
    if (npMode != 0 )
    {
      ...
      protonEndPoint.SetXYZ( evt->CCQENu_proton_endPointX,evt->CCQENu_proton_endPointX,evt->CCQENu_proton_endPointZ);//mm
    }
    ...
    int charge = (neutrinoMode)? -1:1;
    double binding_e = (npMode==0)? 0 : bindingE/1000.;
    ...
    //
    if (isDebug) cout<<"Recoil Characteristics"<<endl;
    ...
    double blobDistToProtonEndPoint = -999;
    bool inVtx = false;
    XYZVector NeutronPos;
    if (isDebug) cout<<"SetBlobInfo"<<endl;
    if( hasNC )
    {
      NeutronPos = (XYZVector) mainCandidate->BlobBegPos.Vect();
    ...
    double pnAngle = -99999;
    int sign = 1;
    double pnEndDist= -9999;
    ...
    if( hasNC && hasProton  && npMode == 10) 
    {
      ...
      duoname = GetDuoCode( beam, reco_muon_XYZ, reco_proton_XYZ, reco_neutron_XYZ );
      //cout<<quadrant<<endl;
      ...
      pnAngle = TMath::ACos( reco_proton_XYZ.Unit().Dot( reco_neutron_XYZ.Unit() ) ) *180/TMath::Pi() ;
      pnEndDist = (NeutronPos - protonEndPoint).R();
      if (pneutron>0)
      {
        ...
        std::pair<int, double>dphittPair = geoUtils->GetDoubleTransversePhiTT( beam, reco_muon_XYZT, primary_nucleon_XYZT, fitNeutron_XYZT, Mcarbon, 27.13/1E3, true );


        sign = dphittPair.first;
        dphitt = dphittPair.second;
    ...
    double angle = 30;
    int blob_multiplicity_015=0;
    if( npMode != 1 )
    {
      blob_multiplicity_015 = GetMultiplicity( ptrNeutronCandidates, angle, 
        blobMult_015, blobUngrouped_015, cones,
        ungroupedE_015, nUngroupedBlobs_015 );
    }
    if( isDebug ) cout<<"Got blob multiplicity"<<endl;
      

    if (isMC)
    {
      if( isDebug ) cout<<"IsMC"<<endl;
      ...
      if(fill_common_histos )
      {
        ...
        if( isDebug ) cout<<"IsMC2-f1"<<endl;
      }

      if(mainCandidate && hasNC)
      {
        if( isDebug ) cout<<"IsMC2-MC"<<endl;
        ...
        if( isDebug ) cout<<"IsMC2-ptr"<<endl;
        ...
        if( isDebug ) cout<<"IsMC2-ptr-end"<<endl;
      }
    }
    ...
    //cout<<"Filling HDL2D"<<endl;
    //Taking HDL out of the fill_common_histos so that it can do its own thing with lateral errorband
    FillHDL2D( "h_dthetaPdthetaR_q2qe", utils, "dthetaP", "q2", "dthetaR", reco_dthetaP, q2qe, reco_dthetaR, isData, evt, wgt, RunCodeWithSystematics,RunLat, pass_cv);//no systematics
    if(hasNC) FillHDL2D( "h_dthetaPdthetaR_q2qe_nc", utils, reco_dthetaP, q2qe, reco_dthetaR, isData, evt, wgt, RunCodeWithSystematics, RunLat, pass_cv);//no systematics
    if( abs(reco_dthetaR)<10 && abs(reco_dthetaP) < 10 )
    {
      FillHDL2D( "h_dthetaPdthetaR_q2qe_fine", utils,"dthetaP", "q2", "dthetaR",  reco_dthetaP, q2qe, reco_dthetaR, isData, evt, wgt, RunCodeWithSystematics, RunLat, pass_cv);//no systematics
      if(hasNC) FillHDL2D( "h_dthetaPdthetaR_q2qe_nc_fine", utils,"dthetaP", "q2", "dthetaR",  reco_dthetaP, q2qe, reco_dthetaR, isData, evt, wgt, RunCodeWithSystematics, RunLat, pass_cv);//no systematics
    }

    //cout<<"Filled HDL2D"<<endl;


    //========================== Histo Filling Proper ============================

    //bool pass10Sel = (blobDist>400) && pnAngle>30 ;
    //bool pass10Sel = (blobDist>400) && pnAngle>30 ;
    //bool pass10Sel = (blobDist>400) && pnAngle>30 ;
    //bool pass10Sel = (blobDist>400) && pnAngle>30 ;
    bool pass10Sel = (blobDist>400 && blobDist<1800) && pnAngle>30 ;
    if( npMode == 10 && !pass10Sel ) continue;
    if (fill_common_histos )
    {
      //---------------------------------------------
      //Fill 1-D Plots
      ...
      if(isDebug) cout<<"h_enu_q2qe"<<endl;
      ...
      utils->fillHistosV3( utils->histos2D["h_dthetaP_dthetaR_VeryFine"],reco_dthetaP,  reco_dthetaR, isData, evt,wgt);
      ...
      if(isDebug) cout<<"h_res_dthetaR"<<endl;
      if(hasNC && hasProton && npMode == 10)
      {

        if(isDebug) cout<<"hasNC && hasProton"<<endl;
        if(isDebug) cout<<"Quadrant is "<<quadrant<<endl;
        ...
        if(isDebug) cout<<"Duo is "<<duoname<<endl;
        utils->fillHistosV3( utils->histos2D[ Form("h_pn_q2qe_%s", duoname.c_str())], reco_pn, q2qe, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dphit_q2qe_%s", duoname.c_str())], reco_dphit, q2qe, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dalphat_q2qe_%s", duoname.c_str())], reco_dalphat, q2qe, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dpt_q2qe_%s", duoname.c_str())], reco_dpt, q2qe, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dptx_q2qe_%s", duoname.c_str())], reco_dptx, q2qe, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dpty_q2qe_%s", duoname.c_str())], reco_dpty, q2qe, isData,evt,wgt);


        utils->fillHistosV3( utils->histos1D[ Form("h_ptmu_%s", duoname.c_str() ) ], ptmu,  isData, evt, wgt );

        if(isDebug) cout<<"a1"<<endl;

        utils->fillHistosV3( utils->histos2D[ Form("h_pn_ptmu_%s", duoname.c_str())], reco_pn, ptmu, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dphit_ptmu_%s", duoname.c_str())], reco_dphit, ptmu, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dalphat_ptmu_%s", duoname.c_str())], reco_dalphat, ptmu, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dpt_ptmu_%s", duoname.c_str())], reco_dpt, ptmu, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dptx_ptmu_%s", duoname.c_str())], reco_dptx, ptmu, isData,evt,wgt);
        utils->fillHistosV3( utils->histos2D[ Form("h_dpty_ptmu_%s", duoname.c_str())], reco_dpty, ptmu, isData,evt,wgt);





        if(isDebug) cout<<"a2"<<endl;
        ...
        utils->fillHistosV3( utils->histos2D[ "h_dphitt_q2qe" ], abs(dphitt), q2qe, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_dphitt_ptmu" ], abs(dphitt), ptmu, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_q2qe" ], dphitt, q2qe, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_ptmu" ], dphitt, ptmu, isData, evt, wgt );

        utils->fillHistosV3( utils->histos2D[ "h_pnAngle_q2qe" ], pnAngle, q2qe, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_pnAngle_ptmu" ], pnAngle, ptmu, isData, evt, wgt );


        utils->fillHistosV3( utils->histos2D[ "h_pnEndDist_q2qe" ], pnEndDist, q2qe, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_pnEndDist_ptmu" ], pnEndDist, ptmu, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_pnEndDist_pnAngle" ], pnEndDist, pnAngle, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_blobDist_pnAngle" ], blobDist, pnAngle, isData, evt, wgt );
        utils->fillHistosV3( utils->histos2D[ "h_blobDist_pnEndDist" ], blobDist, pnEndDist, isData, evt, wgt );

        utils->fillHistosV3( utils->histos1D[ "h_vtx_energy"], evt->vtx_blobs_energy, isData, evt, wgt );
        utils->fillHistosV3( utils->histos1D[ "h_vtx_energy2"], evt->recoil_energy_nonmuon_vtx100mm, isData, evt, wgt );

        //if(isMC && evt->mc_intType==8)
        //{
        //  cout<<"Dist To Vtx: "<<ncutter->DistanceToVtx(evt)<<endl;
        //  cout<<"angle: "<<pnAngle<<endl;
        //  cout<<"vtx blob energy: "<<evt->vtx_blobs_energy<<endl;
        //  cout<<"vtx non muon energy: "<<evt->recoil_energy_nonmuon_nonvtx100mm<<endl;
        //}

        if(sign < 0 )
        {
          utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_q2qe_left" ], dphitt, q2qe, isData, evt, wgt );
          utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_ptmu_left" ], dphitt, ptmu, isData, evt, wgt );
        }
        else
        {
          utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_q2qe_right" ], dphitt, q2qe, isData, evt, wgt );
          utils->fillHistosV3( utils->histos2D[ "h_dphitt_sign_ptmu_right" ], dphitt, ptmu, isData, evt, wgt );
        }

        if(isDebug) cout<<"filled hasNC && hasProton"<<endl;
      }



      if( abs(reco_dthetaR) < 10 || npMode == 10) 
      {
        if(isDebug) cout<<"b1"<<endl;
        ...
        if(isDebug) cout<<"b2"<<endl;


        if(angle_10 || npMode==10)
        {
          ...
          utils->fillHistosV3( utils->histos2D["h_blobEnergy_ptmu"], blobEnergy, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_nonBlobEnergy_ptmu"], nonBlobEnergy, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_blobDist_ptmu"], blobDist, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_nBlobs_ptmu"], nBlobs, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_n2DBlobs_ptmu"], n2DBlobs, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_n3DBlobs_ptmu"], n3DBlobs, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_muonTheta_ptmu"], muon_theta, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_muonPhi_ptmu"], muon_phi_detector, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_muonE_ptmu"], muon_p, ptmu, isData,evt, wgt );

          utils->fillHistosV3( utils->histos2D["h_protonDEDX_ptmu"], proton_score, ptmu, isData,evt, wgt );
          utils->fillHistosV3( utils->histos2D["h_pionDEDX_ptmu"], pion_score, ptmu, isData,evt, wgt );


          if(isDebug) cout<<"b3"<<endl;
          if(mainCandidate)
          {

            if(isDebug) cout<<"b3-1"<<endl;
            ...
            utils->fillHistosV3( utils->histos2D["h_hasTrack_ptmu"], mainCandidate->hasTrack, ptmu, isData,evt, wgt );
            utils->fillHistosV3( utils->histos2D["h_blobMaxE_ptmu"], mainCandidate->ClusMaxE, ptmu, isData,evt, wgt );
            utils->fillHistosV3( utils->histos2D["h_nClus_ptmu"], mainCandidate->NClusters, ptmu, isData,evt, wgt );
            if(isDebug) cout<<"b3-1-end"<<endl;
          }
          ...
          if(isDebug) cout<<"Filled Angle_10"<<endl;
        }


        if( region == 1 )
        {
          if(isDebug) cout<<"b4"<<endl;
          ...
          if(isDebug) cout<<"Filled Region1"<<endl;
        }
        if( region == 5 )
        {
          if(isDebug) cout<<"b5"<<endl;
          ...
          if(isDebug) cout<<"Filled Region5"<<endl;
        }
        if( region == -1 )
        {
          if(isDebug) cout<<"b6"<<endl;
          ...
          if(isDebug) cout<<"Filled Region-1"<<endl;
        }

        if(isDebug) cout<<"Begin filling Sys if RunCodeWithSYstematics is True "<<RunCodeWithSystematics<<endl;
        ...
        if(RunCodeWithSystematics && !isData)
        {
          ...
          if (angle_10)
          {
            utils->fillVertErrorBands( utils->histos2D["h_blobEnergy_q2qe"], blobEnergy, q2qe,evt );
          }
    ...
    } //end fill_common_histos

    if( isMC && RunCodeWithSystematics )
    {
      //These are events with slightly looser cuts
      ...
      //----------------------------------------------------
      utils->fillLatErrorBands( utils->histos1D["h_ptmu"],  "pT",    ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos1D["h_q2qe"],  "q2",    q2qe, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_q2qe_ptmu"],  "q2", "pT",  q2qe, ptmu, evt );
      utils->fillLatErrorBands( utils->histos1D["h_dthetaP"],  "dthetaP", reco_dthetaP, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos1D["h_dthetaR"],  "dthetaR", reco_dthetaR, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos1D["h_pn"],  "pn", reco_pn, evt, pass_cv );
      //This is a test
      utils->fillLatErrorBands( utils->histos2D["h_dthetaP_q2qe"], "dthetaP", "q2", reco_dthetaP, q2qe, evt,pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dthetaR_q2qe"], "dthetaR", "q2", reco_dthetaR, q2qe, evt,pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_pn_q2qe"], "pn", "q2", reco_pn, q2qe, evt,pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dptx_q2qe"], "dpTx", "q2", reco_dptx, q2qe, evt,pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dpty_q2qe"], "dpTy", "q2", reco_dpty, q2qe, evt,pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dpt_q2qe"], "dpT", "q2", reco_dpt, q2qe, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dalphat_q2qe"], "dalphaT", "q2", reco_dalphat, q2qe, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dphit_q2qe"], "dphiT", "q2", reco_dphit, q2qe, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_muontheta_q2qe"], "theta", "q2", muon_theta, q2qe, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_muonmomentum_q2qe"], "emu", "q2", reco_muon_e, q2qe, evt, pass_cv );
      if(angle_10) utils->fillLatErrorBands( utils->histos2D["h_enu_q2qe"], "enu", "q2", enu, q2qe, evt, pass_cv );

      utils->fillLatErrorBands( utils->histos2D["h_dthetaP_ptmu"], "dthetaP", "pT", reco_dthetaP, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dthetaR_ptmu"], "dthetaR", "pT", reco_dthetaR, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_pn_ptmu"], "pn", "pT", reco_pn, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dptx_ptmu"], "dpTx", "pT", reco_dptx, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dpty_ptmu"], "dpTy", "pT", reco_dpty, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dpt_ptmu"], "dpT", "pT", reco_dpt, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dalphat_ptmu"], "dalphaT", "pT", reco_dalphat, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_dphit_ptmu"], "dphiT", "pT", reco_dphit, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_muontheta_ptmu"], "theta", "pT", muon_theta, ptmu, evt, pass_cv );
      utils->fillLatErrorBands( utils->histos2D["h_muonmomentum_ptmu"], "emu", "pT", reco_muon_e, ptmu, evt, pass_cv );
      if(angle_10) utils->fillLatErrorBands( utils->histos2D["h_enu_ptmu"], "enu", "pT", enu, ptmu, evt, pass_cv, pass_cv );

      utils->fillLatErrorBands( utils->histos2D[Form( "h_q2qe_ptmu_region_%02d",regionCode) ], "q2qe","pT", q2qe, ptmu, evt, pass_cv );
      if( region != -1 )utils->fillLatErrorBands( utils->histos2D["h_q2qe_ptmu_region_non99"], "q2qe","pT", q2qe, ptmu, evt, pass_cv );

      utils->fillLatErrorBands( utils->histos2D[Form( "h_enu_q2qe_region_%02d",regionCode) ], "enu","q2qe", enu, q2qe, evt, pass_cv );
      if( region != -1 )utils->fillLatErrorBands( utils->histos2D["h_enu_q2qe_region_non99"], "enu","q2qe", enu, q2qe, evt, pass_cv );

      utils->fillLatErrorBands( utils->histos2D[Form( "h_muontheta_q2qe_region_%02d",regionCode) ], "theta","q2qe", muon_theta, q2qe, evt, pass_cv );
      if( region != -1 )utils->fillLatErrorBands( utils->histos2D["h_muontheta_q2qe_region_non99"], "theta","q2qe", muon_theta, q2qe, evt, pass_cv );
...
int MuonSelectionHists(string filename, string sample, bool makeFluxConstraintHisto, int multiplicity, string playlist, int n_mcfiles = -1, int n_datafiles = -1, int npMode = 1)
{
  ...
  NeutronBlobCuts  *ncutter = (npMode==0)? utils->GetNeutronCutter() : new NeutronBlobCuts();

  if(isDebug) cout<<"Neutron Cutter Address : "<<ncutter<<endl;
  ...
  utils->bookHistos( h_ptmu_g40, "h_ptmu_g40", Form( "Muon Pt;Reconstructed Muon p_{T} (GeV);Events / %.3f GeV", muonPtbins.default_width ), muonPtbins );
  utils->bookHistos( h_ptmu_l40, "h_ptmu_l40", Form( "Muon Pt;Reconstructed Muon p_{T} (GeV);Events / %.3f GeV", muonPtbins.default_width ), muonPtbins );
  ...
  utils->bookHistos( h_pmu_l40, "h_pmu_l40", Form( "Muon Pt;Reconstructed Muon p_{T} (GeV);Events / %.3f GeV", muonPtbins.default_width ), muonPtbins );
  utils->bookHistos( h_pmu_g40, "h_pmu_g40", Form( "Muon Pt;Reconstructed Muon p_{T} (GeV);Events / %.3f GeV", muonPtbins.default_width ), muonPtbins );

  utils->bookHistos( h_vtx_energy, "h_vtx_energy", "Vtx E;Reconstructed Vtx E (MeV) ;Events / 2 MeV", UniformVtxEBins);
  utils->bookHistos( h_vtx_energy2, "h_vtx_energy2", "Vtx E;Reconstructed Vtx E (MeV) ;Events / 2 MeV", UniformVtxEBins);
  ...
  MnvH2D *h_dthetaP_dthetaR_VeryFine[nHistos];
  ...
  utils->bookHistos( h_dthetaP_dthetaR_VeryFine, "h_dthetaP_dthetaR_VeryFine", "dthetaP: dthetaR", dthetaFineBinsUniform, dthetaFineBinsUniform);
...
//  utils->bookHistos( h_protonRes_nodeenergy_node5, "h_protonRes_nodeenergy_node5","res:node",500,0,500,100,-1,1);
  ...
  for( unsigned int i = 0; i< quad_names.size(); i++ )
  {
    ...
    for( unsigned int j = 0; j < stki_reduced_names.size(); j++ )
    {
      ...
      utils->bookHistos( &quad_h_stki_q2qe[i][j][0],
          ...
          stki_reduced_bins[j],Q2bins );
      utils->bookHistos( &quad_h_stki_ptmu[i][j][0],
          Form("h_%s_ptmu_%s", var.c_str(), qn.c_str() ),
          Form("%s_%s:ptmu", var.c_str(), qn.c_str() ),
          stki_reduced_bins[j],muonPtbins );
    }
  }
  vector<string> duo_names({"l40","g40"});
  vector< vector< vector<MnvH2D*> > > 
    duo_h_stki_q2qe( duo_names.size(), vector<vector<MnvH2D*>>( stki_reduced_names.size(), vector<MnvH2D*>(nHistos,NULL) ) ),
    duo_h_stki_ptmu( duo_names.size(), vector<vector<MnvH2D*>>( stki_reduced_names.size(), vector<MnvH2D*>(nHistos,NULL) ) );
  for( unsigned int i = 0; i< duo_names.size(); i++ )
  {
    //h_stki_xvar_ll
    string qn = duo_names[i];
    for( unsigned int j = 0; j < stki_reduced_names.size(); j++ )
    {
      string var = stki_reduced_names[j];
      utils->bookHistos( &duo_h_stki_q2qe[i][j][0],
          Form("h_%s_q2qe_%s", var.c_str(), qn.c_str() ),
          Form("%s_%s:q2qe", var.c_str(), qn.c_str() ),
          stki_reduced_bins[j],Q2bins );
      utils->bookHistos( &duo_h_stki_ptmu[i][j][0],
          Form("h_%s_ptmu_%s", var.c_str(), qn.c_str() ),
          Form("%s_%s:ptmu", var.c_str(), qn.c_str() ),
          stki_reduced_bins[j],muonPtbins );
    }
  ...
  MnvH2D *h_dphitt_sign_q2qe[nHistos], *h_dphitt_sign_ptmu[nHistos];

  MnvH2D *h_dphitt_sign_q2qe_left[nHistos], *h_dphitt_sign_ptmu_left[nHistos];
  MnvH2D *h_dphitt_sign_q2qe_right[nHistos], *h_dphitt_sign_ptmu_right[nHistos];

  MnvH2D *h_pnAngle_q2qe[nHistos], *h_pnAngle_ptmu[nHistos];
  ...
  utils->bookHistos( h_dphitt_sign_q2qe,"h_dphitt_sign_q2qe", "dphitt:q2", dphitbins_sign, Q2bins );
  utils->bookHistos( h_dphitt_sign_q2qe_left,"h_dphitt_sign_q2qe_left", "dphitt:q2", dphitbins_sign, Q2bins );
  utils->bookHistos( h_dphitt_sign_q2qe_right,"h_dphitt_sign_q2qe_right", "dphitt:q2", dphitbins_sign, Q2bins );
  ...
  utils->bookHistos( h_dphitt_sign_ptmu,"h_dphitt_sign_ptmu", "dphitt:ptmu", dphitbins_sign, muonPtbins );
  utils->bookHistos( h_dphitt_sign_ptmu_left,"h_dphitt_sign_ptmu_left", "dphitt:ptmu", dphitbins_sign, muonPtbins );
  utils->bookHistos( h_dphitt_sign_ptmu_right,"h_dphitt_sign_ptmu_right", "dphitt:ptmu", dphitbins_sign, muonPtbins );
  ...
  utils->bookHistos( h_pnAngle_q2qe,"h_pnAngle_q2qe", "pnAngle:q2", dphitbins, Q2bins );
  utils->bookHistos( h_pnAngle_ptmu,"h_pnAngle_ptmu", "pnAngle:ptmu", dphitbins, muonPtbins );
  ...
  MnvH2D *h_blobDist_ptmu[nHistos], *h_blobEnergy_ptmu[nHistos], *h_nBlobs_ptmu[nHistos],*h_n3DBlobs_ptmu[nHistos], *h_n2DBlobs_ptmu[nHistos], *h_muonTheta_ptmu[nHistos], *h_muonPhi_ptmu[nHistos], *h_muonE_ptmu[nHistos], *h_protonDEDX_ptmu[nHistos],*h_pionDEDX_ptmu[nHistos], *h_nonBlobEnergy_ptmu[nHistos], *h_hasTrack_ptmu[nHistos], *h_blobMaxE_ptmu[nHistos], *h_nClus_ptmu[nHistos], *h_blobEnergyLow_ptmu[nHistos];

  utils->bookHistos( h_blobDist_ptmu, "h_blobDist_ptmu", "dist:ptmu", BlobDistBins, muonPtbins);
  utils->bookHistos( h_blobEnergy_ptmu, "h_blobEnergy_ptmu", "blobenergy:ptmu", BlobEnergyBins, muonPtbins);
  utils->bookHistos( h_nonBlobEnergy_ptmu, "h_nonBlobEnergy_ptmu", "nonblobenergy:ptmu", BlobEnergyBins, muonPtbins);
  utils->bookHistos( h_n3DBlobs_ptmu, "h_n3DBlobs_ptmu", "n3dblobs:ptmu", BlobNumBins, muonPtbins );
  utils->bookHistos( h_n2DBlobs_ptmu, "h_n2DBlobs_ptmu", "n2dblobs:ptmu", BlobNumBins, muonPtbins );
  utils->bookHistos( h_nBlobs_ptmu, "h_nBlobs_ptmu", "nblobs:ptmu", BlobNumBins, muonPtbins );
  utils->bookHistos( h_muonTheta_ptmu, "h_muonTheta_ptmu", "muonTheta:ptmu", MuonThetaBins,muonPtbins);
  utils->bookHistos( h_muonPhi_ptmu, "h_muonPhi_ptmu", "muonPhi:ptmu",MuonPhiBins, muonPtbins );
  utils->bookHistos( h_muonE_ptmu, "h_muonE_ptmu", "muonE:ptmu",MuonEnergyBins, muonPtbins );
  utils->bookHistos( h_protonDEDX_ptmu, "h_protonDEDX_ptmu", "protonDEDX:ptmu",10,-1,1,6,0.1,0.4);
  utils->bookHistos( h_pionDEDX_ptmu, "h_pionDEDX_ptmu", "pionDEDX:ptmu",10,-1,1,6,0.1,0.4);
  utils->bookHistos( h_hasTrack_ptmu, "h_hasTrack_ptmu", "hasTrack:ptmu",2,-.5,1.5,6,0.1,0.4);
  utils->bookHistos( h_blobMaxE_ptmu, "h_blobMaxE_ptmu", "blobMaxE:ptmu",BlobEnergyBins, muonPtbins);
  utils->bookHistos( h_nClus_ptmu, "h_nClus_ptmu", "blobMaxE:ptmu",nClusBins, muonPtbins);
  utils->bookHistos( h_blobEnergyLow_ptmu, "h_blobEnergyLow_ptmu", "blobenergy:ptmu", BlobEnergyBinsLow, muonPtbins);
  ...
  MnvH2D *h_pnEndDist_ptmu[nHistos], *h_pnEndDist_q2qe[nHistos];
  MnvH2D *h_pnEndDist_pnAngle[nHistos];
  MnvH2D *h_blobDist_pnAngle[nHistos];
  MnvH2D *h_blobDist_pnEndDist[nHistos];
  utils->bookHistos( h_pnEndDist_q2qe,"h_pnEndDist_q2qe", "pnEndDist:q2", BlobDistBins, Q2bins );
  utils->bookHistos( h_pnEndDist_ptmu,"h_pnEndDist_ptmu", "pnEndDist:ptmu", BlobDistBins, muonPtbins );
  utils->bookHistos( h_pnEndDist_pnAngle,"h_pnEndDist_pnAngle", "pnEndDist:pnAngle", BlobDistBins, dphitbins );
  utils->bookHistos( h_blobDist_pnAngle,"h_blobDist_pnAngle", "blobDist:pnAngle", BlobDistBins, dphitbins );
  utils->bookHistos( h_blobDist_pnEndDist,"h_blobDist_pnEndDist", "blobDist:pnEndDist", BlobDistBins, BlobDistBins );
  ...
  if(RunCodeWithSystematics){
    cout<<"Adding Systematic Universes"<<endl;
      utils->addVertErrorBands( h_ptmu );
      utils->addLatErrorBands ( h_ptmu );

      utils->addVertErrorBands( h_ptmu_g40 );
      utils->addLatErrorBands ( h_ptmu_g40 );

      utils->addVertErrorBands( h_ptmu_l40 );
      utils->addLatErrorBands ( h_ptmu_l40 );
      ...
      utils->addVertErrorBands( h_dphitt_sign_q2qe );
      utils->addVertErrorBands( h_dphitt_sign_ptmu );
      utils->addVertErrorBands( h_dphitt_sign_q2qe_left );
      utils->addVertErrorBands( h_dphitt_sign_q2qe_right );
      utils->addVertErrorBands( h_dphitt_sign_ptmu_left );
      utils->addVertErrorBands( h_dphitt_sign_ptmu_right );
      ...
      utils->addLatErrorBands( h_dphitt_sign_q2qe );
      utils->addLatErrorBands( h_dphitt_sign_ptmu );

      utils->addLatErrorBands( h_dphitt_sign_q2qe_left );
      utils->addLatErrorBands( h_dphitt_sign_q2qe_right );
      utils->addLatErrorBands( h_dphitt_sign_ptmu_left );
      utils->addLatErrorBands( h_dphitt_sign_ptmu_right );
      ...
      utils->addVertErrorBands( h_blobEnergy_q2qe );



      utils->addVertErrorBands( h_pnAngle_q2qe );
      utils->addVertErrorBands( h_pnAngle_ptmu );
      utils->addLatErrorBands( h_pnAngle_q2qe );
      utils->addLatErrorBands( h_pnAngle_ptmu );

      cout<<"Added systematics"<<endl;
...
//int MuonSelectionHists(string filename, string sample, bool makeFluxConstraintHisto, int multiplicity, string playlist, int n_mcfiles = -1, int n_datafiles = -1, int npMode = 1)
  ...
  modify_Eb = bool(atoi(par[10].c_str() ));
