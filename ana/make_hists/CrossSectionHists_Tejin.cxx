...
bool doHydrogenScale = true;

bool doesNotUseBetter = true;

double POTScale = 4.42;
//double POTScale = -1;
...
bool AddTargetNormalizationSys(MnvH1D* &xs);
...
TMatrixD UnfoldDummy( MnvH1D* h_data_nobck, MnvH1D* h_data_unfolded, MnvH1D* h_reco, MnvH1D* h_truth, MnvH2D* h_migration, int num_iter ) 
{
  ...
  if( doesNotUseBetter) unfold.setUseBetterStatErrorCalc(0);
...
void SetCovToMnvH1D( MnvH1D* h_data_unfolded, TMatrixD &cov, double potScale=1 )
{
  double scale = Sqrt( 1+1/abs(potScale) );
  if( potScale < 0 ) scale = 1;
  for( int i = 1; i< h_data_unfolded->GetNbinsX()+1; i++ )
  {
    double err = h_data_unfolded->GetBinError(i);
    err*=abs(scale);
    h_data_unfolded->SetBinError(i, err );
  }
  cov*=abs(scale);

  h_data_unfolded->PushCovMatrix("unfoldingCov",cov);
}
...
bool UnfoldData( MnvH1D* &h_data_unfolded, MnvH1D* h_data_nobck, MnvH1D* h_reco, MnvH1D* h_truth, MnvH2D* h_migration, int num_iter ) 
{
  ...
  SetCovToMnvH1D( h_data_unfolded, unfoldingCovMatrixOrig, POTScale );
...
int CrossSectionHists( string output_filename, string filename_signal_regions, string filename_bg_weights, string filename_migration_histogram, string filename_eff, int num_iter, bool makeFluxConstraintHisto, bool applyFluxConstraint,int run_type,bool doenu)
{
  ...
  if(doHydrogenScale) h_q2qe_signal[kQELike_QE_H]->Scale(hydrogen_scale);
  ...
  if(doHydrogenScale)
  {
    h_effhist_num[kQELike_QE_H]->Scale(hydrogen_scale);
    h_effhist_dem[kQELike_QE_H]->Scale(hydrogen_scale);
  ...
  if( doHydrogenScale)
  {
    h_migration->Scale(hydrogen_scale);
    h_reco->Scale(hydrogen_scale);
    h_truth->Scale(hydrogen_scale);
  }
  ...
  //-------------------------------------------------------
  MnvH1D *h_data_cross_section, *h_mc_cross_section, *h_mc_cross_section_raw;
  ...
  if(doenu){
    ...
    //enu
    cout << "Enu Norm" << endl;
    MnvH1D* h_effhist_demH = (MnvH1D*) h_effhist_dem[kQELike_QE_H]->Clone("h_effhist_demH");
    ...
    NormalizeByFluxAndTargets( h_mc_cross_section_raw, h_effhist_demH, h_flux, pot_data, applyFluxConstraint, utils, h_flux_normalization, true );
    ...
    //NormalizeByFluxAndTargets( h_enu_data_cross_section, h_enu_data_effcor, h_flux, pot_data, applyFluxConstraint, utils, h_flux_normalization2D);
    //NormalizeByFluxAndTargets( h_enu_mc_cross_section, h_enu_mc_effcor, h_flux, pot_data, applyFluxConstraint, utils, h_flux_normalization2D );
    ...
    NormalizeByFluxAndTargets( h_mc_cross_section_raw, h_effhist_demH, h_flux, pot_data, applyFluxConstraint, utils,NULL, true );

    //cout<<"normalize enu-q2"<<endl;

    //NormalizeByFluxAndTargets( h_enu_data_cross_section, h_enu_data_effcor, h_flux, pot_data, applyFluxConstraint, utils);
    //NormalizeByFluxAndTargets( h_enu_mc_cross_section, h_enu_mc_effcor, h_flux, pot_data, applyFluxConstraint, utils);
  ...
  AddTargetNormalizationSys(h_data_cross_section);
  ...
  h_mc_cross_section_raw->Write("h_mc_cross_section_raw");
  h_mc_cross_section_raw->Write();
...
void NormalizeByFluxAndTargets(MnvH1D*&h_normalized,MnvH1D*h_effcor, MnvH1D* h_flux, double pot_scale, bool applyFluxConstraint, CCQENuUtils* utils, MnvH1D*flux_norm, bool isMC )
{
  ...
  double flux_cv = h_flux->Integral( b_min, b_max, "width" );
  //double flux_cv = h_flux->Integral( b_min, b_max );
  ...
  */
  ...
  if( isMC && doHydrogenScale ) targets*=hydrogen_scale;
  cout<<"N Hydrogen Targets: "<<targets<<endl;
...
void NormalizeByFluxAndTargets( MnvH2D *&h_normalized, MnvH2D *h_effcor, MnvH1D* h_flux, double pot_scale, bool applyFluxConstraint, CCQENuUtils* utils, MnvH2D *flux_norm , bool isMC)
{
  ...
  */
  ...
  double targets = 0.0;
  targets=TargetUtils::Get().GetTrackerNNucleons( nplanes, /*isMC =*/ isMC, 850 );
...
bool AddTargetNormalizationSys(MnvH1D* &xs){
  TH1D* cv_xs = (TH1D*)xs->GetCVHistoWithStatError().Clone("tmp_cv_hist");
  TH1D* sys_xs_up = (TH1D*) cv_xs->Clone("sys_up");
  TH1D* sys_xs_down = (TH1D*) cv_xs->Clone("sys_down");
  double sys = 0.009;
  sys_xs_up->Scale(1+sys);
  sys_xs_down->Scale(1-sys);
  vector<TH1D*> norm_sys({sys_xs_up, sys_xs_down});

  xs->AddVertErrorBand("HydrogenTargetNorm", norm_sys);
...
int main( int argc, char *argv[])
{
  ...
  par.push_back("0"); //flux
  par.push_back("4"); //num_iter
  par.push_back("-1"); //runType
  par.push_back("1");//9 doENU
